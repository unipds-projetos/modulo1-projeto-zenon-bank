...
public class TransactionSQLRepository implements TransactionRepository{
    ...
    public void save(Transaction transaction) {
        String sql = """
                ...
                (step, `type` , amount, name_origin, old_balance_origin, new_balance_origin, name_recepient, old_balance_recepient, new_balance_recepient, is_fraud, is_flagged_fraud)
                values
                (?,?,?,?,?,?,?,?,?,?, ?);
                """;
    ...
    public Optional<Transaction> findByOriginName(String originName) {
        ...
        String sql = """
                SELECT id, step, `type`, amount, name_origin, old_balance_origin,
                       new_balance_origin, name_recepient, old_balance_recepient, 
                       new_balance_recepient, is_fraud, is_flagged_fraud
    ...
    private Transaction mapResultSetToTransaction(ResultSet rs) {
        try {
            ...
            String recipientName = rs.getString("name_recepient");
            BigDecimal recipientOldBalance = rs.getBigDecimal("old_balance_recepient");
            BigDecimal recipientNewBalance = rs.getBigDecimal("new_balance_recepient");
